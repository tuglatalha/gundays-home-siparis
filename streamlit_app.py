...
def _a1(row: int, col: int) -> str:
    """1-indexed row/column bilgisini A1 hücre adresine çevirir."""
    letters = ""
    while col:
...
def values_to_df(values, default_headers) -> pd.DataFrame:
    """Google Sheets değerlerini güvenli DataFrame'e çevirir."""
    headers = default_headers[:] if default_headers else []
    if values:
        first_row = [str(x).strip() for x in values[0]]
        if any(first_row):
            headers = first_row
            # Şablondaki yeni kolonlar varsa sonuna ekle
...
@st.cache_resource(show_spinner=False)
def worksheet_registry():
    """Worksheet listesini tek sefer okuyup cache'ler. Böylece her sekme için ayrı okuma yapılmaz."""
    sh = open_spreadsheet()
    return {ws.title: ws for ws in sh.worksheets()}


def get_or_create_ws(name: str):
    registry = worksheet_registry()
    if name in registry:
    ...
    if name in SHEET_HEADERS:
        ws.update("A1", [SHEET_HEADERS[name]])
    worksheet_registry.clear()
    return ws


@st.cache_data(ttl=120, show_spinner=False)
def batch_read_all_sheets():
    """Tüm sekmeleri mümkün olduğunca tek API isteğiyle okur. Kota taşmasını ciddi azaltır."""
    sh = open_spreadsheet()
    names = list(SHEET_HEADERS.keys())
    ranges = [f"{name}!A1:Z10000" for name in names]
    data = {}
    try:
        result = sh.values_batch_get(ranges)
        value_ranges = result.get("valueRanges", [])
        for name, value_range in zip(names, value_ranges):
            data[name] = values_to_df(value_range.get("values", []), SHEET_HEADERS[name])
        # Herhangi bir range dönmediyse boş DataFrame ile tamamla
        for name in names:
            data.setdefault(name, pd.DataFrame(columns=SHEET_HEADERS[name]))
    except Exception:
        # Eski/uyumsuz gspread sürümleri için güvenli fallback. Normalde buraya düşmez.
        for name in names:
            ws = get_or_create_ws(name)
            data[name] = values_to_df(ws.get_all_values(), SHEET_HEADERS[name])
    return data


def initialize_book():
    """Eksik sekmeleri oluşturur. Her sayfa yenilemede değil, oturumda bir kez çalışır."""
    if st.session_state.get("workbook_initialized"):
        return

    for name in SHEET_HEADERS:
        get_or_create_ws(name)

    data = batch_read_all_sheets()

    # Default user
    users = data.get("Kullanicilar", pd.DataFrame(columns=SHEET_HEADERS["Kullanicilar"]))
    if users.empty:
    ...
    # Default lists
    lists_df = data.get("Listeler", pd.DataFrame(columns=SHEET_HEADERS["Listeler"]))
    if lists_df.empty:
        rows = []
        ...
        ws.append_rows([[r.get(h, "") for h in SHEET_HEADERS["Listeler"]] for r in rows], value_input_option="USER_ENTERED")
        st.cache_data.clear()

    st.session_state.workbook_initialized = True


def read_sheet(name: str) -> pd.DataFrame:
    return batch_read_all_sheets().get(name, pd.DataFrame(columns=SHEET_HEADERS.get(name, []))).copy()


def append_row(name: str, data: dict):
    ws = get_or_create_ws(name)
    headers = SHEET_HEADERS.get(name) or read_sheet(name).columns.tolist()
...
def update_row_by_id(sheet_name: str, id_col: str, id_value: str, updates: dict) -> bool:
    ...
    for col_name, value in updates.items():
        if col_name in headers:
            ...
            update_payload.append({"range": _a1(target_row, col_idx), "values": [[value]]})
    if update_payload:
...
def delete_rows_matching(sheet_name: str, match_col: str, match_value: str) -> int:
    ...
    if df.empty or match_col not in df.columns:
        return 0
    rows_to_delete = (df.index[df[match_col].astype(str).str.strip() == str(match_value).strip()] + 2).tolist()
    for row_no in sorted(rows_to_delete, reverse=True):
        ws.delete_rows(int(row_no))
    if rows_to_delete:
        st.cache_data.clear()
...
@st.cache_data(ttl=120, show_spinner=False)
def load_all_data():
    data = {name: df.copy() for name, df in batch_read_all_sheets().items()}
    for name in SHEET_HEADERS:
        data.setdefault(name, pd.DataFrame(columns=SHEET_HEADERS[name]))
